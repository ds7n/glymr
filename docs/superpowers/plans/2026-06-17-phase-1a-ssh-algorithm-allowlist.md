...
**Architecture:** A new private module `algorithms` inside the `glymr-ssh-core` crate. It depends only on `russh`'s type surface (`Preferred` + the `kex`/`cipher`/`mac`/`compression` name constants + `ssh_key::Algorithm`), not on any async runtime — so it compiles and tests in the Docker dev container with zero network. The function is `pub(crate)`: Phase 1b's connection code consumes it; nothing is exported over UniFFI in this sub-plan.

**Tech Stack:** Rust (stable), `russh` 0.61.2 (pin `russh = "0.61"`), the Glymr dev container (`docker compose run --rm dev cargo test`).
...
| File | Responsibility |
|---|---|
| `crates/glymr-ssh-core/Cargo.toml` | Add `russh = "0.61"` dependency |
| `crates/glymr-ssh-core/src/algorithms.rs` | `build_preferred()`, `TIER3_WIRE_NAMES`, `is_tier3()`, unit tests |
| `crates/glymr-ssh-core/src/lib.rs` | Add `mod algorithms;` |
...
- Create: `crates/glymr-ssh-core/src/algorithms.rs`
...
In `crates/glymr-ssh-core/Cargo.toml`, under `[dependencies]` (below the existing `uniffi` line):
```toml
russh = "0.61"
```
...
In `crates/glymr-ssh-core/src/lib.rs`, add after the `uniffi::setup_scaffolding!();` line:
```rust
mod algorithms;
```
...
Create `crates/glymr-ssh-core/src/algorithms.rs` with the header, the test helpers, and the three Tier-1 tests — **but no `build_preferred` yet** (so the tests fail to compile, the Rust equivalent of red):
```rust
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
...
Add to `crates/glymr-ssh-core/src/algorithms.rs`, above the `#[cfg(test)]` module:
```rust
use std::borrow::Cow;
...
```bash
git add crates/glymr-ssh-core/Cargo.toml Cargo.lock crates/glymr-ssh-core/src/lib.rs crates/glymr-ssh-core/src/algorithms.rs
...
**Files:**
- Modify: `crates/glymr-ssh-core/src/algorithms.rs`
...
Run: `HOST_UID=$(id -u) HOST_GID=$(id -g) docker compose run --rm dev cargo test -p glymr-ssh-core legacy_toggle_adds_tier2_only`
...
```bash
git add crates/glymr-ssh-core/src/algorithms.rs
...
```bash
git add crates/glymr-ssh-core/src/algorithms.rs docs/superpowers/specs/2026-06-17-ssh-algorithms-design.md
...
- [ ] `cargo test -p glymr-ssh-core` green in the dev container (all allowlist tests + `core_version`).
