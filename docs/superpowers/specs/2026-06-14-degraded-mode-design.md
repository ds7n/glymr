...
- **No auto-install / bootstrap of tmux.** Glymr never drops binaries on a user's host, never runs package managers, never sudoes. If tmux isn't there, we degrade.
...
- **No power-user "raw" toggle.** Users who want raw SSH access have other clients (Blink, Termius, Prompt 3). Glymr's product story is tmux-native.
