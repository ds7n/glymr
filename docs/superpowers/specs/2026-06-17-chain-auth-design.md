...
2. **Cancel a mid-chain Face ID prompt** (iOS-native cancel on the biometric sheet) — Glymr aborts the in-flight connection: closes the SSH socket to the current hop, closes any sockets to earlier hops, marks the connection as failed in the picker, fires the existing connect-failed banner from [[2026-06-16-banner-expanded-design]] with the message *"Authentication cancelled at hop {n}."*
