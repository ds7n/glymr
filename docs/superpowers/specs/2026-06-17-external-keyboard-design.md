...
This spec covers Glymr's behavior when a hardware keyboard is connected — Bluetooth or USB-C on iPhone, Magic Keyboard / Smart Keyboard Folio on iPad. iOS suppresses the software keyboard automatically in this state; Glymr adapts the keybar and accepts raw key events from the hardware.
...
This is the convention every iOS terminal client documents and the path users already know. Glymr documents it in **Tips & Gestures** and in **About & Help → Keyboard tips**.
...
- **Font-size shortcuts (⌘+ / ⌘−)** — deferred. Glymr has no font-size feature specced anywhere yet; adding the shortcut implies adding the feature. Revisit when font-size lands.
