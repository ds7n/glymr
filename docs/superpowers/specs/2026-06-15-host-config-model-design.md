...
> **Scope.** This spec defines the **data schema** for a Glymr host record: fields, types, defaults inheritance, identity references, jump chains, port forwards, mosh/Tailscale extensions, `known_hosts` storage, and the storage backbone the schema lives on.
>
...
1. **`ssh_config(5)`-faithful naming and semantics.** Every field that has an OpenSSH analog uses the OpenSSH name and means what OpenSSH means by it. Glymr extensions live under clearly namespaced fields (`mosh.*`, `tailscale.*`, `glymr.*`) so they cannot be confused with SSH-derived options.
...
3. **Lossless import/export between Glymr schema and `~/.ssh/config`** is a design goal for the SSH-derived fields. An imported host round-trips through export → import without losing information for any field we model. Glymr extensions are skipped or emitted as comments on export.
...
6. **End-to-end encryption everywhere, regardless of whether the user has Apple's "Advanced Data Protection" enabled.** Glymr applies client-side AES-GCM to CloudKit records so Apple sees only ciphertext.
...
| `glymr.predictor.incognito` | `false` |
| `glymr.tmux.attemptControlMode` | `true` |
...
- Glymr extensions are namespaced: `mosh.*`, `tailscale.*`, `glymr.*` — never intermixed with OpenSSH-derived fields at the top level.
...
- **At rest in iCloud:** Apple sees CloudKit record count, sizes, modification timestamps. Record contents are AES-GCM ciphertext (Glymr's client-side encryption). The encryption key lives in iCloud Keychain (E2EE; Apple cannot read).
