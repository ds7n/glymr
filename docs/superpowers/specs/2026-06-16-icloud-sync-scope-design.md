...
The principle gives a clean default for future additions: when a new piece of user data is added to Glymr, classify it as configuration (user-authored choices) or behavior/history (runtime artifacts) and apply this rule. Exceptions like the predictor sketches need explicit justification (and an updated spec).
...
This is the macro-level equivalent of the per-host `glymr.predictor.incognito` flag from the host-config-model. Same mental model, same opt-out shape, same purpose: let users keep sensitive entries local without forcing them to disable the whole sync category.
...
1. User installs Glymr on a new device, signs into iCloud, opens the app.
...
For the "I wiped local data on this device, please re-sync" case, the same flow applies — re-signing into iCloud / re-installing Glymr / re-launching pulls the current synced state. No explicit restore button.
...
- A user signing into Glymr on a second Apple device finds their hosts, identities (non-Secure-Enclave), snippets, keybar customizations, and predictor vocabulary already there — no restore action needed.
...
- A user wiping the device or buying a new one experiences "open Glymr → it's all there" rather than "open Glymr → empty → import → resolve → painful."
